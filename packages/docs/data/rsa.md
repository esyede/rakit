...
`RSA` encrypts and decrypts data with a private / public key pair.

> Key pairs generated by this component are always RSA 2048-bit.
...
> If no keys have been loaded yet, a private key and public key are generated automatically on the first
> operation and reused for the rest of the request. They only live in memory, so if you need to decrypt the
> data in another request, [export](#export-keys) the keys and [load](#load-keys) them back later.
...
Without loaded keys, a new pair is generated once per request. `load_keys()` supplies your own:
...
Export the keys for storage or sharing:
...
- config      : Contents of the temporary openssl config used while generating
- options     : Options passed to openssl, including key size and type
*/

echo $details['options']['private_key_bits']; // 2048
```

> `config` and `options` are only filled in when the key pair was generated by
> Rakit itself. They stay empty when the keys were supplied through
> [RSA::load_keys()](#load-keys).
...
```php
// Server side - Share public key
// (keys are generated per request, so load the same stored keys on every request)
Route::get('api/public-key', function () {
    RSA::load_keys(Storage::get('keys/private.pem'), Storage::get('keys/public.pem'));
...
// Or specify padding type (use the same padding when decrypting)
...
```php
// A single RSA 2048-bit block holds max ~245 bytes, so RSA::encrypt()
// splits larger data into many blocks, which is slow.
// For large data, encrypt a random AES key with RSA
// then encrypt the data with that AES key

$aes_key = Str::random(32);
$iv = Str::bytes(16);
$encrypted_key = RSA::encrypt($aes_key);
$encrypted_data = openssl_encrypt($large_data, 'aes-256-cbc', $aes_key, 0, $iv);
