### Getting Started
- [Overview](/docs/home)
- [Release Notes](/docs/changelog)
- [Instalation & Initial Configuration](/docs/install) <!-- has-submenu -->
    - [System Requirements](/docs/install#kebutuhan-sistem)
    - [Instalation](/docs/install#instalasi)
    - [Initial Configuration](/docs/install#konfigurasi-awal)
    - [Pretty URL](/docs/install#mempercantik-url)
- [Directory Structure](/docs/structure)
- [Routing](/docs/routing) <!-- has-submenu -->
    - [Basic Knowledge](/docs/routing#pengetahuan-dasar)
    - [URI Wildcard](/docs/routing#uri-wildcard)
    - [Event 404](/docs/routing#event-404)
    - [Middleware](/docs/routing#middleware)
    - [URI Pattern Middleware](/docs/routing#middleware-pola-uri)
    - [Global Middleware](/docs/routing#middleware-global)
    - [Route Grouping](/docs/routing#route-grouping)
    - [Named Route](/docs/routing#named-route)
    - [Package Routing](/docs/routing#routing-paket)
    - [Controller Routing](/docs/routing#routing-controller)
    - [Route Testing via CLI](/docs/routing#route-testing-via-cli)
- [Controller](/docs/controllers) <!-- has-submenu -->
    - [Basic Knowledge](/docs/controllers#pengetahuan-dasar)
    - [Routing Controller](/docs/controllers#controller-routing)
    - [Package Controller](/docs/controllers#controller-paket)
    - [Action Middleware](/docs/controllers#action-middleware)
    - [Nested Controller](/docs/controllers#nested-controller)
    - [RESTful Controller](/docs/controllers#restful-controller)
    - [Dependency Injection](/docs/controllers#dependency-injection)
    - [Controller Factory](/docs/controllers#controller-factory)
- [View & Response](/docs/views) <!-- has-submenu -->
    - [Basic Knowledge](/docs/views#pengetahuan-dasar)
    - [Binding Data to View](/docs/views#binding-data-ke-view)
    - [Nested View](/docs/views#nested-view)
    - [Named View](/docs/views#named-view)
    - [Composer View](/docs/views#view-composer)
    - [Redirect](/docs/views#redirect)
    - [Redirect with Flash Data](/docs/views#redirect-dengan-flash-data)
    - [Download Response](/docs/views#respon-download)
    - [Error Response](/docs/views#respon-error)
    - [Managing Assets](/docs/views/assets)
    - [Templating](/docs/views/templating)
    - [Pagination](/docs/views/pagination)
- [Input & Cookies](/docs/input) <!-- has-submenu -->
    - [Input](/docs/input#input)
    - [File](/docs/input#file)
    - [Old Input](/docs/input#old-input)
    - [Redirect with Old Input](/docs/input#redirect-dengan-old-input)
    - [Cookies](/docs/input#cookies)
- [Package](/docs/packages) <!-- has-submenu -->
    - [Basic Knowledge](/docs/packages#pengetahuan-dasar)
    - [Creating Package](/docs/packages#membuat-paket)
    - [Package & Autoloading](/docs/packages#paket--autoloading)
    - [Booting Package](/docs/packages#booting-paket)
    - [Routing to Package](/docs/packages#routing-ke-paket)
    - [Using Package](/docs/packages#menggunakan-paket)
    - [Paket Assets](/docs/packages#aset-paket)
    - [Installing Package](/docs/packages#menginstall-paket)
    - [Upgrading Package](/docs/packages#mengupgrade-paket)
    - [Deleting Package](/docs/packages#menghapus-paket)
- [Autoloading](/docs/autoloading)
- [Error & Debugging](/docs/debugging)
- [Runtime Configuration](/docs/config)
- [Request Checking](/docs/requests)
- [Creating URL](/docs/urls)
- [Event & Listener](/docs/events)
- [Data Validation](/docs/validation)

### Database

- [Configuration](/docs/database/config)
- [Raw Query](/docs/database/raw)
- [Magic Query Builder](/docs/database/magic)
- [Facile Model](/docs/database/facile)
- [Schema Builder](/docs/database/schema)
- [Database Migration](/docs/database/migrations)
- [Using Faker](/docs/database/faker)
- [Redis](/docs/database/redis)

### Caching

- [Configuration](/docs/cache/config)
- [Usage](/docs/cache/usage)

### Session

- [Configuration](/docs/session/config)
- [Usage](/docs/session/usage)

### Authentication

- [Configuration](/docs/auth/config)
- [Usage](/docs/auth/usage)

### Support

- [Helper](/docs/helpers)
- [String](/docs/strings)
- [Array](/docs/arrays)
- [Date](/docs/date)
- [Image](/docs/image)
- [Storage](/docs/storage)
- [Crypter](/docs/crypter)
- [Localization](/docs/localization)
- [Markdown](/docs/markdown)
- [Email](/docs/email)
- [Container](/docs/container)
- [cURL](/docs/curl)
- [Console](/docs/console)
- [Job](/docs/job)
