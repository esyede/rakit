...
class SQLServer extends Grammar
{
    ...
    /**
     * Compile a fulltext index. The catalog is named after the index and made only when
     * missing; without ->key(), the table's primary key is the unique key index.
     *
     * @param Table $table
     * @param Magic $command
     *
    ...
    public function fulltext(Table $table, Magic $command)
    {
        ...
        // The server names the primary key, so look it up. EXEC() takes only literals
        // and variables, hence the @sql variable.
    ...
    /**
     * Compile the drop of a fulltext catalog, but only once no index still uses it.
     *
     * @param Magic $command
     *
     * @return string
